...
[ $# != 0 ] && echo "Usage: local/fph_data_prep.sh" && exit 1;

srcdir=data/local/train  
dir=data/local/dict_nosilp
rm -fr $dir
mkdir -p $dir
...
cat $dir/lexicon1.txt | awk '{ for(n=2;n<=NF;n++){ phones[$n] = 1; }} END{for (p in phones) print p;}' | \
  grep -v "\(SIL\|SPN\|FLL\|LAU\)" > $dir/nonsilence_phones.txt  || exit 1;

#( echo sil; echo spn; echo nsn; echo lau ) > $dir/silence_phones.txt
( echo SIL; echo SPN; echo FLL; echo LAU ) > $dir/silence_phones.txt
...
echo Prepared input dictionary and phone-sets for FSC+KIT phase 1.
